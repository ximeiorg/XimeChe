...
pub async fn pair_request(
    ...
    if req.device_id.is_empty() || req.device_name.is_empty() {
        return Err(ApiError::InvalidRequest(
            "device_id and device_name required".into(),
        ));
    }
    ...
    Ok(Json(PairRequestResponse { code, expires_in }))
}
...
pub async fn pair_list(State(state): State<ServerState>) -> Json<DeviceListResponse> {
