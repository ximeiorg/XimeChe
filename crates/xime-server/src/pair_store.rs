...
impl PairStore {
    ...
    pub fn create_session(&mut self, device_id: String, device_name: String) -> PairSession {
        ...
        self.pending_sessions.insert(session.code.clone(), session.clone());
        session
    }
    ...
    pub fn confirm_session(&mut self, code: &str, token: String) -> Result<PairedDevice, ApiError> {
        let session = self.pending_sessions.get_mut(code)
        ...
        self.paired_devices.insert(device.device_id.clone(), device.clone());
